...
public static class BFS
{
	...
	public static bool Search<TNode>(
		...
		Action<IEnumerable<TNode>>? between = null,
	...
	) where TNode : notnull
	{
		bool success = Search<TNode, UnitDirection>(
			...
			between is not null ? (p, _, _) => between(p.Select(p => p.Item2)) : null,
			nodeComparer
		);
	...
	public static bool Search<TNode, TDirection>(
		...
		Action<IEnumerable<(TDirection, TNode)>, TNode>? between = null,
		IEqualityComparer<TNode>? nodeComparer = null
	) where TNode : notnull
	{
		bool success = Search<TNode, TDirection>(
			...
			between is not null ? (p, n, _) => between(p.Select(p => (p.Item1, p.Item2)), n) : null,
			nodeComparer
		);
	...
	public static bool Search<TNode>(
		...
		Action<IEnumerable<(TNode, long)>, long>? between = null,
		IEqualityComparer<TNode>? nodeComparer = null
	) where TNode : notnull
	{
		bool success = Search<TNode, UnitDirection>(
			...
			goalCondition,
			out IEnumerable<(UnitDirection, TNode, long)> internalPath,
			between is not null ? (p, _, c) => between(p.Select(p => (p.Item2, p.Item3)), c) : null,
			nodeComparer
		);
	...
	public static bool Search<TNode, TDirection>(
		...
		Action<IEnumerable<(TDirection, TNode, long)>, TNode, long>? between = null,
		IEqualityComparer<TNode>? nodeComparer = null
	) where TNode : notnull
	{
		...
		while (toVisit.Count > 0)
		{
			...
			{
				between(pathToCurrent, current, cost);
			}
